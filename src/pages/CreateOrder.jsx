...
export default function CreateOrder() {
  ...
  // Load data dengan Auto-Merge (CreateOrder.jsx)
  useEffect(() => {
    loadCustomers();
    
    // ========== DATA PRODUK DEFAULT (9 PRODUK LENGKAP) ==========
    ...
    if (!savedProductsStr) {
      // Kasus 1: localStorage kosong → simpan defaultProducts langsung
      setProducts(defaultProducts);
      localStorage.setItem('products', JSON.stringify(defaultProducts));
      console.log('✅ Auto-Merge: localStorage kosong, menyimpan 9 produk default');
    } else {
      // Kasus 2: localStorage sudah ada isinya → lakukan merge
      const currentProducts = JSON.parse(savedProductsStr);
      
      // Buat Set untuk ID produk yang sudah ada (pencarian cepat)
      const existingIds = new Set(currentProducts.map(p => p.id));
      
      // Filter produk default yang BELUM ada di currentProducts
      const newProductsToAdd = defaultProducts.filter(dp => !existingIds.has(dp.id));
      
      if (newProductsToAdd.length > 0) {
        // Gabungkan produk lama dengan produk baru yang belum ada
        const mergedProducts = [...currentProducts, ...newProductsToAdd];
        
        // Simpan hasil merge ke localStorage
        localStorage.setItem('products', JSON.stringify(mergedProducts));
        setProducts(mergedProducts);
        
        console.log(`✅ Auto-Merge: Menambahkan ${newProductsToAdd.length} produk baru ke localStorage`);
        console.log('📦 Produk baru yang ditambahkan:', newProductsToAdd.map(p => p.name).join(', '));
      } else {
        // Tidak ada produk baru, gunakan data yang sudah ada
        setProducts(currentProducts);
        console.log('✅ Auto-Merge: Semua produk default sudah ada, tidak ada perubahan');
      }
    }
